# ====================================================
# FEERv1.0-G1.2: bundle daily NetCDFs, or to GeoTiff
...
FEERv1_pro <- function(varName, xYears, xMonths, outputType="tif") {
  
  invName <- "FEERv1p0_G1p2"
  ...
  readFEER <- function(ncin,idx) {
    ncvar <- ncvar_get(ncin,orig_varName,start=c(1,1,idx),
                       count=c(ncin$dim$lon$len,ncin$dim$lat$len,1))
    lat <- ncvar_get(ncin,"lat")
    lon <- ncvar_get(ncin,"lon")
    
    ncdfras <- raster(t(ncvar[order(lon),rev(order(lat))]))
    ncdfras[is.na(ncdfras)] <- 0
  ...
  for (iYear in seq_along(xYears)) {
    setwd(inputFolder)
    invYrName <- paste0("FEERv1.0-G1.2_Daily_",xYears[iYear],".nc")
    ncin <- quiet(nc_open(invYrName))
    ...
    for (iMonth in xMonths) {
      ...
      for (iDay in seq_along(monthDays)) {
        ...
        invDay_ras <-  readFEER(ncin,monthDays[iDay])
...
for (varName in varNameL) {
  FEERv1_pro(varName, xYears, xMonths, outputType)
}
