# ========================================
# GFASv1.2: reformat monthly NetCDFs
...
GFASv1p2_nc <- function(varName, xYears, xMonths) {
  ...
  for (iYear in seq_along(xYears)) {
    for (iMonth in xMonths) {
      ...
      for (iDay in seq_along(monthDays)) {
        ...
        invDay_vec <- readGFAS(invMonName,ncvar,iDay)
...
for (varName in varNameL) {
  GFASv1p2_nc(varName, xYears, xMonths)
}
